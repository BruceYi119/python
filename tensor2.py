...
#     # print(result[0],result[1][0],result[2][0])
...
# b=tf.fill((5,1),-1.)
# y=tf.matmul(x,w)+b   #(5,1)
# with tf.Session() as sess:
#     print(sess.run(y,feed_dict={x:xdata,w:wdata}))
# tf.argmax() :가장 큰값을 찾아 인덱스 반환--------------
# a=tf.constant([10,100,1,1000])
# b=tf.constant([[10,100,1],[11,21,31],[0,80,50]])
# with tf.Session() as sess:
#     # print('인덱스갯수(차원)',sess.run(tf.rank(a)))
#     # print('가장 큰값 위치',sess.run(tf.argmax(a)))
#     # print('가장 작은값 위치',sess.run(tf.argmin(a)))
#     print('인덱스갯수(차원)',sess.run(tf.rank(b)))
#     print(sess.run(b))
#     print('가장 큰값 위치:열방향',sess.run(tf.argmax(b)))
#     print('가장 큰값 위치:열방향',sess.run(tf.argmax(b,0)))
#     print('가장 큰값 위치:행방향',sess.run(tf.argmax(b,1)))
#---행방향으로 가장큰값의 위치 출력
# a=tf.constant([[5,10,17],[4,50,6]])
# with tf.Session() as sess:
#     print(sess.run(a))
#     # print('가장 큰값 위치(열방향)',sess.run(tf.argmax(a)))
#     print('가장 큰값 위치(행방향)',sess.run(tf.argmax(a,1)))
# -원핫:사람이 이해할수 있는 데이터를 컴에게 입력-------
# 0:1000000000
# 5:0000010000
# import numpy as np
...
# onehotx=tf.one_hot(x,10)     #차원이 늘어남
# # onehotx2=tf.reshape(onehotx,shape=[-1])  #차원축소
...
#     print(' y=\n',sess.run(y,feed_dict={y:b}))
#     print('원핫으로 y=\n',sess.run(onehoty,feed_dict={y:b}))
#     print('원래값으로 y=\n',sess.run(argmaxy,feed_dict={y:b}))

# r에서 iris데이터를 받아 현재 프로젝트의 data폴더내에 iris.csv로 넣으세요
