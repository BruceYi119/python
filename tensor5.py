...
data = np.loadtxt(os.path.join('data','zoo.csv'), delimiter=',')
...
y = tf.compat.v1.placeholder(tf.int32,[None,1])   #0-6  ==>onehot
onehot = tf.compat.v1.one_hot(y, 7)
onehot2 = tf.reshape(onehot, [-1,7])
w = tf.Variable(tf.compat.v1.random_normal([16,7]))
b = tf.Variable(tf.compat.v1.random_normal([7]))
# 예측값
logits = tf.matmul(x,w) + b
h = tf.nn.softmax(logits)
cost = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=onehot2)
train = tf.compat.v1.train.GradientDescentOptimizer(0.7).minimize(cost)
with tf.compat.v1.Session() as sess:
    ...
    # print(sess.run(accuracy, feed_dict={ x: xdata, y: ydata }))
    # 예측하기
